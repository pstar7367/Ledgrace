...
export const createSavingsGoal = async (req, res) => {
  ...
  if (!name?.trim() || !Number.isFinite(target) || target <= 0) {
    return res.status(400).json({
      message: "A goal name and target amount greater than zero are required.",
    });
  }
  if (!Number.isFinite(saved) || saved < 0 || saved > target) {
    return res.status(400).json({
      message: "Saved amount must be zero or no more than the target amount.",
    });
  }
  if (targetDate && Number.isNaN(new Date(targetDate).getTime())) {
    return res
      .status(400)
      .json({ message: "Please provide a valid target date." });
  }
  ...
  await createNotification({
    ...
    detail: `${goal.name} with a ${target.toLocaleString("en-NG", { style: "currency", currency: "NGN" })} target is now being tracked.`,
    source: "goal",
    sourceId: goal.id,
  });
...
export const updateSavingsGoal = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "A valid goal name and target amount are required." });
  }
...
export const addGoalContribution = async (req, res) => {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    return res
      .status(400)
      .json({ message: "Contribution amount must be greater than zero." });
  }
  ...
  if (goal.paused)
    return res
      .status(400)
      .json({ message: "Resume this goal before adding a contribution." });
  if (goal.savedAmount >= goal.targetAmount)
    return res
      .status(400)
      .json({ message: "This goal has already been completed." });
  ...
  await createNotification({
    user: req.user.id,
    type: goal.savedAmount >= goal.targetAmount ? "achievement" : "update",
    title:
      goal.savedAmount >= goal.targetAmount
        ? "Goal milestone achieved"
        : "Goal contribution added",
    detail: `${appliedAmount.toLocaleString("en-NG", { style: "currency", currency: "NGN" })} was added to ${goal.name}.`,
    source: "goal",
    sourceId: goal.id,
  });
  ...
  res.json({
    message:
      appliedAmount < amount
        ? "The contribution completed this goal; only the remaining amount was added."
...
export const deleteSavingsGoal = async (req, res) => {
  ...
  await createNotification({
    ...
    detail: `${goal.name} was removed from your goals.`,
    source: "goal",
    sourceId: goal.id,
  });
  res.status(204).send();
