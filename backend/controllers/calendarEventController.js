...
const validateEvent = ({ title, date, amount }) => {
  ...
  if (!date || Number.isNaN(new Date(date).getTime())) return "A valid event date is required.";
...
export const getCalendarEvents = async (req, res) => {
  const events = await CalendarEvent.find({ user: req.user.id }).sort({ date: 1, createdAt: -1 });
  res.json({ events });
};
