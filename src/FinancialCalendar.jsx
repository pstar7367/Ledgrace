...
export default function FinancialCalendar({ topSearch = "" }) {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ||
          "Unable to load your financial calendar.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("ledgrace:open-calendar-event", openEventForm);
  }, []);

  const financialEvents = useMemo(() => {
    const billEvents = bills.flatMap((bill) =>
      recurringDatesForMonth(bill, viewDate).map((date) => ({
  ...
  const summary = useMemo(() => {
    ...
    const upcoming = financialEvents.filter(
      (event) =>
        event.source === "bill" &&
        event.status !== "paid" &&
        event.date >= today &&
    ...
    const paid = bills.filter((bill) => {
      ...
      return (
        paidDate.getFullYear() === viewDate.getFullYear() &&
        paidDate.getMonth() === viewDate.getMonth()
      );
    });
    const overdue = financialEvents.filter(
      (event) =>
        event.source === "bill" &&
        event.status !== "paid" &&
        event.date < today,
  ...
  const saveEvent = async (event) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ||
          "Unable to save your calendar event.",
      );
    }
  };
  ...
  const removeEvent = async (id) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ||
          "Unable to delete this calendar event.",
      );
    }
  };
  ...
  const renderEvent = (event, compact = false) => (
    <article className={`financial-event ${event.kind}`} key={event.id}>
      <span
        className="financial-event-dot"
        style={{ background: eventColors[event.kind] }}
      />
      <div>
        <b>{event.title}</b>
      ...
      {event.amount > 0 && (
        <strong className={event.kind === "income" ? "income" : ""}>
          {event.kind === "income" ? "+" : ""}
      ...
      {event.source === "custom" && (
        <button
          type="button"
          onClick={() => removeEvent(event.originalId)}
    ...
    <section className="financial-calendar-page">
      <div className="calendar-heading">
        ...
        <div className="calendar-heading-actions">
          ...
          <label
            className="budget-date-chip calendar-date-chip"
      ...
      <div className="calendar-layout">
          ...
          {loading ? (
            ...
            >
              ...
              {(viewMode === "week"
                ...
                return (
                  <div
                    className={`calendar-day ${isCurrentMonth ? "" : "outside"} ${isSameDay(day, new Date()) ? "today" : ""}`}
                    key={day.toISOString()}
                  >
        ...
        <aside className="calendar-side">
              ...
              <div className="calendar-side-list">
                {upcomingList.map((event) => (
                  <div key={event.id} className="calendar-side-item">
                    <span
                      style={{
                        background: `${eventColors[event.kind]}20`,
                        color: eventColors[event.kind],
                      }}
                    >
          ...
          <section className="calendar-side-card">
            ...
            {summary.overdue.length ? (
              summary.overdue.slice(0, 4).map((event) => (
                <div key={event.id} className="calendar-side-item overdue">
                  <span>!</span>
                  <div>
                    <b>{event.title}</b>
                    <small>{formatDay(event.date)}</small>
  ...
  return (
    ...
    >
      <form onSubmit={onSubmit}>
        ...
        <h2>Add Event</h2>
        <p>Add a personal financial reminder to your calendar.</p>
        <label>
          Event name
          <input
            required
