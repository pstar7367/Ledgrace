...
function Metric({ label, value, icon: Icon, good, expense }) {
  return (
    <article className="dash-metric">
      <span className={expense ? "expense" : good ? "income" : ""}>
        <Icon />
      </span>
      <small>{label}</small>
...
export default function DashboardPage() {
    ...
    const loadWorkspaceTransactions = async () => {
      ...
      try {
        ...
        const savedClientIds = new Set(
          savedTransactions.map((item) => item.clientId),
        );
        const unsyncedLegacyTransactions = legacyTransactions.filter(
          (item) => !savedClientIds.has(item.clientId || item.id),
        );

        if (unsyncedLegacyTransactions.length) {
          await Promise.all(
            unsyncedLegacyTransactions.map((item) =>
              createTransactionRequest({
                ...item,
          ...
          if (isCurrent) {
            setTransactions(
              refreshed.data.transactions.map(toDashboardTransaction),
            );
          }
          return;
        }
  ...
  useEffect(() => {
    const removeDeletedTransaction = (event) => {
      ...
      setTransactions((items) =>
        items.filter((item) => item.id !== event.detail.id),
      );
    };
  ...
  const toggleTopNotificationRead = async (notification) => {
    try {
      if (notification.read) {
        await markNotificationUnreadRequest(notification._id);
      } else {
      ...
      setWorkspaceError("Unable to update this notification.");
    }
  };

  const toggleTopNotificationsRead = async () => {
    try {
      ...
      setNotifications((items) =>
        items.map((item) => ({ ...item, read: hasUnread })),
      );
    } catch {
      setWorkspaceError("Unable to mark notifications as read.");
    }
  };
  ...
  const removeTransaction = async (id) => {
    try {
      ...
      setWorkspaceError(
        error.response?.data?.message ||
          "Unable to remove this transaction. Please try again.",
      );
    }
  };
    ...
    <div className="app-dashboard">
      ...
      <main>
                ...
                <div className="notification-dropdown">
                  ...
                  {notifications.slice(0, 6).map((item) => (
                    <button
                      ...
                      key={item._id}
                      onClick={() => toggleTopNotificationRead(item)}
                    >
                      <span>
                        <Bell size={14} />
                      </span>
            ...
            {active !== "Accounts" &&
              ...
              (active === "Bills & Subscriptions" ? (
                <button
                  ...
                  onClick={() =>
                    window.dispatchEvent(
                      new CustomEvent("ledgrace:open-bill-form"),
                    )
                  }
                >
                ...
                <button
                  ...
                  onClick={() =>
                    window.dispatchEvent(
                      new CustomEvent("ledgrace:open-calendar-event"),
                    )
                  }
                >
                ...
                <button
                  ...
                  onClick={() =>
                    window.dispatchEvent(
                      new CustomEvent("ledgrace:open-goal-form"),
                    )
                  }
                >
        ...
        <section className="dash-content">
          {workspaceError && (
            <p className="dash-workspace-status">{workspaceError}</p>
          )}
          {active !== "Accounts" &&
            active !== "Income" &&
            active !== "Expenses" &&
          ...
          {active === "Accounts" ? (
            <AccountsManager topSearch={search} />
          ) : active === "Income" ? (
            <IncomeManager
              topSearch={search}
            ...
            <BillsAndSubscriptionsManager topSearch={search} />
          ) : active === "Financial Calendar" ? (
            <FinancialCalendar topSearch={search} />
          ) : active === "Analytics" ? (
            <Analytics topSearch={search} />
          ) : active === "Reports" ? (
