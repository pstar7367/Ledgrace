...
export default function IncomeManager({ topSearch = "", onAddIncome, period: controlledPeriod, onPeriodChange }) {
  ...
  const loadIncome = async () => {
    ...
    try {
      ...
      setIncome(data.transactions.filter((transaction) => transaction.type === "income"));
    } catch (requestError) {
      setError(
        requestError.response?.data?.message || "Unable to load your income records. Please try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const applyTransactionChange = (event) => {
      ...
      if (action === "created" && transaction?.type === "income") {
        setIncome((items) => [transaction, ...items.filter((item) => item._id !== transaction._id)]);
      }
      if (action === "deleted") {
  ...
  const searchedIncome = useMemo(() => {
    ...
    return periodIncome.filter((item) => (
  ...
  const summary = useMemo(() => {
    ...
    const highest = periodIncome.reduce((largest, item) => (
  ...
  const recurringSources = sources.filter((source) => (
  ...
  const donutStyle = sources.length && summary.total
    ? {
      background: `conic-gradient(${sources.map((source, index) => {
        const start = sources.slice(0, index).reduce((sum, item) => sum + item.amount, 0) / summary.total * 100;
  ...
  const deleteIncome = async (id) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Unable to delete this income record.");
    }
  };
  ...
  return (
    <section className="income-manager">
      ...
      <div className="income-stats">
        <IncomeStat label={`Total Income (${periodLabel})`} value={money.format(summary.total)} icon={WalletCards} />
        <IncomeStat label="Average Monthly Income" value={money.format(summary.average)} icon={BarChart3} />
        <IncomeStat label="Highest Income" value={summary.highest ? money.format(summary.highest.amount) : "—"} note={summary.highest?.title || "No income recorded"} icon={TrendingUp} />
        <IncomeStat label="Income Transactions" value={periodIncome.length} icon={CircleDollarSign} />
      </div>

      {loading ? (
        <div className="income-empty"><WalletCards /><h2>Loading income records…</h2></div>
      ) : !periodIncome.length ? (
        <div className="income-empty">
          <WalletCards />
          <h2>{income.length ? `No income for ${periodLabel.toLowerCase()}` : "Add your first income"}</h2>
          <p>{income.length ? "Choose another date range or add a new income record." : "Record a salary, freelance payment, business income, or any other money you receive."}</p>
          <button className="button primary" onClick={onAddIncome}><Plus size={17} /> Add Income</button>
        </div>
      ) : (
        <>
          {activeTab !== "Recurring Income" && (
            <div className="income-overview-grid">
              <section className="income-panel income-trend-panel">
                <div className="income-panel-title"><h2>Income Overview</h2>{renderPeriodPicker()}</div>
                <strong className="income-total">{money.format(summary.total)}</strong>
                <p>Income recorded for {periodLabel.toLowerCase()}</p>
                <div className="income-chart" aria-label="Income trend for the past seven days">
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img">
                    <polyline points={chartPoints} fill="none" stroke="#00a978" strokeWidth="2.4" vectorEffect="non-scaling-stroke" />
                    {trend.map((point, index) => {
                      const x = 6 + index * (88 / 6);
                      const y = 90 - (point.value / maxTrend) * 76;
                      return <circle key={point.label} cx={x} cy={y} r="2.2" fill="#fff" stroke="#00a978" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />;
                    })}
                  </svg>
              ...
              <section className="income-panel income-sources-panel">
                <div className="income-panel-title"><h2>Income by Source</h2>{renderPeriodPicker()}</div>
                <div className="income-donut-wrap">
                  ...
                  <div className="income-source-legend">{sources.slice(0, 5).map((source) => <div key={source.name}><i style={{ background: source.color }} /><span>{source.name}</span><b>{Math.round(source.amount / summary.total * 100)}%</b></div>)}</div>
                </div>
              </section>
            </div>
          )}

          {activeTab === "Sources" && <SourceSummary sources={sources} total={summary.total} />}
          {activeTab === "Recurring Income" && <SourceSummary sources={recurringSources} total={summary.total} recurring />}
...
function SourceSummary({ sources, total, recurring = false }) {
  if (!sources.length) {
    return <div className="income-empty compact"><CalendarDays /><h2>{recurring ? "No recurring income yet" : "No income sources found"}</h2><p>{recurring ? "Record the same source more than once to see it here." : "Add an income record to see its source here."}</p></div>;
  }

  return <section className="income-panel income-source-summary"><div className="income-panel-title"><h2>{recurring ? "Recurring Income Sources" : "All Income Sources"}</h2></div>{sources.map((source) => <div className="income-source-row" key={source.name}><i style={{ background: source.color }} /><b>{source.name}</b><span>{money.format(source.amount)}</span><em>{Math.round(source.amount / total * 100)}%</em></div>)}</section>;
}

function IncomeTable({ income, onDelete }) {
  return <section className="income-panel income-table-panel">
    <div className="income-panel-title"><h2>Recent Income Transactions</h2><span>{income.length} record{income.length === 1 ? "" : "s"}</span></div>
    {!income.length ? <p className="income-no-results">No income records match your search.</p> : <div className="income-table">{income.slice(0, 12).map((item) => <article className="income-row" key={item._id}><span className="income-row-icon"><BriefcaseBusiness /></span><div><b>{item.title}</b><small>{formatTransactionDate(item)}</small></div><em>{item.category}</em><strong>+{money.format(item.amount)}</strong><button onClick={() => onDelete(item._id)} aria-label={`Delete ${item.title}`}><Trash2 size={16} /></button></article>)}</div>}
  </section>;
}
